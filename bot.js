...
client.on("message", async message => {
  if (activeUsers.has(message.author.id)) {
      if (await db.fetch(`spamCount_${message.author.id}`)) {
        if (
          (await db.fetch(`spamCount_${message.author.id}`)) >
          3)
         {
          let role = message.guild.roles.get(
            await db.fetch(`muteRole_${message.guild.id}`)
          );
          if (!role) return;
          message.channel.send({
            embed: {
              title: client.user.username + " | Spam Koruması",
              color: client.colors.purple,
              description:
                "Bu sunucu `" +
                client.user.username +
                " Spam Koruması` ile korunmaktadır. " +
                message.author +
                ", spam yapmayı bırak!",
              footer: {
                text: client.user.username + " | Spam Koruması",
          ...
          message.guild.owner.send({
            embed: {
              ...
              color: client.colors.purple,
              description:
                "Sunucunda bir spam saldırısı yaşandı. Sorumlu kullanıcı: `" +
                message.author.tag +
                " | " +
                message.author.id +
                "`",
              fields: [
                {
                  name: "Uygulanan İşlem",
                  value: "`Susturulmuş` Rolü verildi."
                }
              ],
              footer: {
                text: client.user.username + " | Spam Koruması",
  ...
  db.add(`spamCount_${message.author.id}`, 1);
  setTimeout(() => {
    activeUsers.delete(message.author.id);
    db.delete(`spamCount_${message.author.id}`);
  }, 1000 * 3);
});
